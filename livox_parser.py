...
def parse_version_1_1(fd, verbose = 0):
    ...
    xyzi_file_idx = 0
    rot_pitch = R.from_euler('y', 30, degrees=True)
    while True:
        ...
        # print(frame_points)
        xyzit_points = np.array([[pts[0], pts[1], pts[2], pts[3], pts[4]] for pts in frame_points])
        xyzi_points = np.array([[pts[0], pts[1], pts[2], pts[3]] for pts in frame_points])
        xyz_points = np.array([rot_pitch.apply([pts[0], pts[1], pts[2]]) for pts in frame_points])
        global OUT_DIR
        # print(xyzi_points)
        # print(rot_pitch.apply([1, 1, 1]))
        #np.savetxt('{:0>4d}.xyz'.format(xyzi_file_idx), xyzi_points)
        #np.savetxt('{:0>4d}_rot.xyz'.format(xyzi_file_idx), xyz_points)
        #np.savetxt('{}/{:0>4d}.xyz'.format(OUT_DIR, xyzi_file_idx), xyzit_points)
        out_fname = '{}/{:0>4d}.xyz'.format(OUT_DIR, xyzi_file_idx)
        with open(out_fname, 'w') as f_out_xyz:
...
if __name__ == "__main__":
    global OUT_DIR
    if len(sys.argv) != 3:
        print('Usage: {} <lvx_filepath> <out_dir>'.format(sys.argv[0]))
        sys.exit(1)
    #if len(sys.argv) == 2:
